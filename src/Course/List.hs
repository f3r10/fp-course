...
-- filter f = foldRight(\x ys -> if (f x) then (x:.ys) else ys) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional xs = foldRight(twiceOptional(:.))(Full Nil) xs
...
find ::
  ...
  -> List a
  -> Optional a
-- find f xs = foldRight(\x acc-> if (f x) then Full(x) else acc) Empty xs
find f ys = case filter f ys of
              Nil -> Empty
              head:._ -> Full(head)
